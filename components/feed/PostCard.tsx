...
export default function PostCard({ post, onLike, onComments, onBookmark }: PostCardProps) {
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 flex gap-4 text-sm text-slate-600">
        ...
        <button onClick={() => onComments(post.id)}>Comments {post._count?.comments ?? post.commentCount ?? 0}</button>
